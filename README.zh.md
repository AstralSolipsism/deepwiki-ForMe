...
DeepWiki 提供 MCP Server，通过 StreamableHTTP 传输方式对外暴露仓库查询和检索能力。

### 可用工具

| 工具 | 描述 |
|------|------|
| `ask_repo` | 仓库问答（支持RAG和多模型流式） |
| `retrieve_context` | 检索相关代码片段 |
| `list_processed_projects` | 列出所有已处理的项目 |
| `get_local_repo_structure` | 读取本地仓库结构 |
...
| 环境变量 | 默认值 | 描述 |
|---------|--------|------|
| `DEEPWIKI_MCP_HOST` | `127.0.0.1` | MCP服务监听地址 |
| `DEEPWIKI_MCP_PORT` | `8000` | MCP服务端口 |
| `DEEPWIKI_MCP_PATH` | `/mcp` | MCP端点路径 |
| `DEEPWIKI_API_BASE_URL` | `http://localhost:8001` | DeepWiki后端地址 |
| `DEEPWIKI_API_TIMEOUT` | `300` | API超时时间（秒） |

### MCP 客户端配置

要连接 DeepWiki MCP Server，在MCP客户端中配置：
...
[![星标历史图表](https://api.star-history.com/svg?repos=AstralSolipsism/deepwiki-ForMe&type=Date)](https://star-history.com/#AstralSolipsism/deepwiki-ForMe&Date)
