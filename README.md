...
DeepWiki provides an MCP Server that exposes repository query and retrieval capabilities via the Model Context Protocol with StreamableHTTP transport.

### Available Tools

| Tool | Description |
|------|-------------|
| `ask_repo` | Repository Q&A with RAG and multi-model streaming |
| `retrieve_context` | Retrieve relevant code snippets |
| `list_processed_projects` | List all processed projects |
| `get_local_repo_structure` | Read local repository structure |
...
|---------------------|---------|-------------|
| `DEEPWIKI_MCP_HOST` | `127.0.0.1` | MCP server host |
| `DEEPWIKI_MCP_PORT` | `8000` | MCP server port |
| `DEEPWIKI_MCP_PATH` | `/mcp` | MCP endpoint path |
| `DEEPWIKI_API_BASE_URL` | `http://localhost:8001` | DeepWiki backend URL |
| `DEEPWIKI_API_TIMEOUT` | `300` | API timeout in seconds |

### MCP Client Configuration

To connect to the DeepWiki MCP Server, configure your MCP client with:
...
[![Star History Chart](https://api.star-history.com/svg?repos=AstralSolipsism/deepwiki-ForMe&type=Date)](https://star-history.com/#AstralSolipsism/deepwiki-ForMe&Date)
